    ...
    public partial class frmSelection : Form
    {
        ...
        private void frmSelection_Load(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in selectedStatus.Rows)
            {
                for (int i = 1; i <= 55; i++)
                {
                    if (row["s" + i.ToString()] is bool && !(bool)row["s" + i.ToString()])
                    {
                        countZeros++;
                    }
                }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (clickedRow.Cells.Count > 0 && clickedRow.Cells[0].Value != null)
                {
                    ...
                    {
                        string busNumberValue = busDetails2.Rows[0]["Bus Number"].ToString();
                        travelDate = busDetails2.Rows[0]["Travel Date & Time"].ToString();
                        string arrivalPlaceValue = busDetails2.Rows[0]["To"].ToString();
                        string availableSeatsValue = busDetails2.Rows[0]["Available Seats"].ToString();
                        ...
                        frmBooking.travelDate.Text = travelDate;
                        ...
                        busNumber = busDetails2.Rows[0]["Bus Number"].ToString();
