...
namespace commuterLiners.Forms
{
    public partial class frmAvailability : Form
    {
        ...
        {
            DataTable dtValidation = DBHelper.ValidateBus(dateTimePicker1.Value.ToString("dd/MM/yyyy"), comboBoxTo.Text.Trim());

            if (dtValidation.Rows.Count > 0)
            {
                DBHelper.InsertTicketNumber();
                frmSelection frmSelection = new frmSelection();
                frmSelection.TravelDate = dateTimePicker1.Value.ToString("dd/MM/yyyy");
