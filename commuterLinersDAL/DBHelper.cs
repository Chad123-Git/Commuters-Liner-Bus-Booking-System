...
namespace commutersLinersDAL
{
    public class DBHelper : ConnectionLine
    {
        ...
        public static DataTable SelectByDate(string dateToSelect)
        {
            Database db = null;
            DbCommand dbCommand = null;
            try
            {
                db = GetCLDatabase();
                dbCommand = db.GetStoredProcCommand("SelectByDate");
                db.AddInParameter(dbCommand, "@DateToSelect", DbType.String, dateToSelect);
                DataTable dt = db.ExecuteDataSet(dbCommand).Tables[0];
                return dt;
            }
            catch { return null; }
